...
function calculate_clip_size(width, height) {
  ...
  var _ref = width < height ? [0 - margin, max / 2 - min / 2 - margin, min + margin * 2, min + margin * 2] // ソースのクリッピング領域
...
function load_fisheye_image_canvas_texture(url) {
  ...
  return load_image(url).then(function (img) {
    ...
    var _calculate_clip_size = calculate_clip_size(width, height, margin);
...
function load_fisheye_video_canvas_texture(url) {
  ...
  return load_video(url).then(function (video) {
    ...
    var _calculate_clip_size2 = calculate_clip_size(videoWidth, videoHeight, margin);
...
function create_camera(type) {
  ...
  : new THREE.PerspectiveCamera(45, WINDOW_WIDTH / WINDOW_HEIGHT, 1, 10000);
  console.log(camera);
...
var WINDOW_WIDTH = exports.WINDOW_WIDTH = window.innerWidth;
var WINDOW_HEIGHT = exports.WINDOW_HEIGHT = WINDOW_WIDTH / 4;

function updateAngleOfView(camera, renderer, mesh) {
  exports.WINDOW_WIDTH = WINDOW_WIDTH = window.innerWidth;
  exports.WINDOW_HEIGHT = WINDOW_HEIGHT = WINDOW_WIDTH / 4;
