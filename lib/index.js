...
function createPanoramaMesh(panorama_width) {
  // パノラマ板ポリの空間上の横幅
  return function _createPanoramaMesh(fisheye_texture) {
    // 正方形テクスチャを仮定
    var R1_ratio = 0; // 扇型の下弦 0~1
    var R2_ratio = 1; // 扇型の上弦 0~1 下弦 < 上弦
    var h_per_w_ratio = function () {
    ...
    faceVertexUvs[0] = faceVertexUvs[0].map(function (pt2Dx3) {
      return pt2Dx3.map(function (_ref5) {
        var x = _ref5.x;
        var y = _ref5.y;
        var xD = x;
        var yD = y;
...
function _main() {
  ...
  // 魚眼素材と表示方法をひとつ選択
  load_fisheye_image_canvas_texture(src).then(createFisheyeMesh)]).then(function (_ref6) {
